...
Every served page also carries a strict CSP and `X-Robots-Tag: noai, noimageai, noindex`, so a published page is not indexed and is marked off-limits for training. The CSP allows Google Fonts and Loom/YouTube/Vimeo/Wistia/Descript/Calendly embeds, and blocks remote scripts and remote images — inline scripts are hashed at publish time and do run, so inline them rather than linking them. Design for that policy rather than around it: if a publish would break something, the response comes back with a `warnings` array naming it. If the user asks for permanence, a public slug, or relaxed PII checking, surface the option rather than quietly disabling a default.
